...
function decodeNotePlaintext(bytes) {
    if (bytes.length !== PLAINTEXT_LEN)
        throw new Error("bad note plaintext");
...
/// wallet only needs `s`. The X25519 secret is `sha256("pl2:enc" || s)`.
export function encryptionKeypair(spendingKey) {
    const seed = sha256(new Uint8Array([...new TextEncoder().encode("pl2:enc"), ...frToBytes(spendingKey)]));
...
export function encryptNote(note, recipientEncPk) {
...
export function tryDecryptNote(encrypted, spendingKey) {
    if (encrypted.length < 56 + 16)
        return null;
    try {
...
export class NoteStore {
    ...
    unspent(assetId) {
        return this.notes.filter((n) => !n.spent && (assetId === undefined || n.assetId === assetId));
    }
    balance(assetId) {
        return this.unspent(assetId).reduce((sum, n) => sum + n.amount, 0n);
    }
    toJSON() {
        return JSON.stringify(this.notes.map((n) => ({
...
export async function scanNoteEvent(store, spendingKey, leafIndex, commitment, encryptedNote) {
    ...
    if (decrypted.ownerPk !== pk)
        return null;
    const recomputed = await noteCommitment(decrypted.assetId, decrypted.amount, decrypted.ownerPk, decrypted.blinding);
