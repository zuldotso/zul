...
#   sudo systemctl stop zul-node@mainnet
#   gunzip -c <backup>.redb.gz > /home/ubuntu/ZUL/data/mainnet/zul.redb
#   sudo systemctl start zul-node@mainnet
#
# Schedule nightly via cron, e.g.:
#   0 4 * * *  ZUL_NET=mainnet ZUL_BACKUP_REMOTE=b2:zul-backups /home/ubuntu/ZUL/scripts/backup-redb.sh
set -euo pipefail

NET=${ZUL_NET:-testnet}
DATA=${ZUL_DATA:-/home/ubuntu/ZUL/data/$NET}
DB="$DATA/zul.redb"
DEST=${ZUL_BACKUP_DIR:-/home/ubuntu/ZUL/backups/$NET}
